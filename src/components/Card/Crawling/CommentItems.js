...
export default function CommentItems({ comments, deleteComment, changeText }) {
    ...
    <div className={styles.commentItems__container}>
      {comments ? comments.map((comment) => (
        ...
        <CommentItem key={comment.id}>
          ...
          <CommentItem.Button self={authStore?.user?.id === comment.user?.id} deleteComment={deleteComment(comment.id)} changeText={changeText} value={comment.user?.name} />
        </CommentItem>
      )) : []}
